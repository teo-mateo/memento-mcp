import { VectorStore, VectorSearchResult } from '../../types/vector-store.js';
...
export class Neo4jVectorStore implements VectorStore {
   ...
   */
  private calculateVectorStats(vector: number[]): {min: number, max: number, avg: number, l2Norm: number} {
    let min = Infinity;
    let max = -Infinity;
    let sum = 0;
    let sumSquared = 0;
    ...
    try {
      ...
      try {
        ...
        const indexInfo = indexResult.records.length > 0 
          ? { name: indexResult.records[0].get('name'), state: indexResult.records[0].get('state') } 
        ...
        try {
          ...
          directVectorQueryResult = {
            ...
            sampleResult: testResult.records.length > 0 ? 
              { name: testResult.records[0].get('node.name'), score: testResult.records[0].get('score') } : null
          };
        } catch (error) {
          directVectorQueryResult = {
            success: false,
