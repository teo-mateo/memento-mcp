...
interface MockStorageProvider {
  ...
  // Vector search
  searchVectors(embedding: number[], limit?: number, threshold?: number): Promise<Array<{ name: string; score: number }>>;
...
describe('KnowledgeGraphManager integration with EmbeddingJobManager', () => {
  ...
  it('should use the embedding service for semantic search', async () => {
    ...
    expect(mockStorageProvider.searchVectors).toHaveBeenCalledWith(
      expect.any(Array),
      5,
      0.7
    );
