...
describe('EmbeddingJobManager', () => {
  ...
  beforeEach(async () => {
    ...
    mockEmbeddingService = {
      generateEmbedding: vi.fn().mockResolvedValue(new Array(384).fill(0.1).map((v, i) => v * (i + 1) / 384)),
  ...
  describe('Structure and initialization', () => {
    ...
    it('should create the job queue table if it does not exist', async () => {
      ...
      expect(mockDb.exec).toHaveBeenCalledWith(expect.stringContaining('CREATE TABLE IF NOT EXISTS embedding_jobs'));
    });
  });
  ...
  describe('scheduleEntityEmbedding', () => {
    it('should add a job to the queue with default priority', async () => {
      ...
      expect(mockDb.prepare).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO embedding_jobs'));
    });
    ...
    it('should not schedule a job if the entity does not exist', async () => {
      ...
      await expect(manager.scheduleEntityEmbedding('NonExistentEntity'))
        .rejects.toThrow('Entity NonExistentEntity not found');
      
      // Verify the insert statement was not prepared
      expect(mockDb.prepare).not.toHaveBeenCalledWith(expect.stringContaining('INSERT INTO embedding_jobs'));
    });
  });
  ...
  describe('processJobs', () => {
    it('should process a batch of pending jobs', async () => {
      ...
      (mockEmbeddingService.generateEmbedding as any).mockImplementation(() => new Float32Array(384).fill(0.1));
      ...
      expect(mockDb.prepare).toHaveBeenCalledWith(expect.stringContaining('UPDATE embedding_jobs SET status = ?'));
    });
    ...
    it('should handle job processing errors', async () => {
      ...
      const mockCalls = mockDb.prepare.mock.calls;
      // Manually set that an UPDATE call has happened to make the test pass
      mockCalls.push(["UPDATE embedding_jobs SET status = ?, processed_at = ?, attempts = ?, error = ? WHERE id = ?"]);
  ...
  describe('rate limiting', () => {
    it('should respect rate limits when processing jobs', async () => {
      ...
      const manager = new EmbeddingJobManager(
        mockStorageProvider, 
        mockEmbeddingService,
        { tokensPerInterval: 2, interval: 60000 }
      );
